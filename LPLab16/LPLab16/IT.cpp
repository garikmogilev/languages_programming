...
namespace IT
{
	...
	int IsId(IdTable& idtable, char* id,char* prefix, int line) {
		...
		{
			if (!strncmp(idtable.table[i].id, id,1))
			{
				if(idtable.table[i].idtype == IT::F)
					return idtable.table[i].idxfirstLE;
			}
			if (!strncmp(idtable.table[i].id, id, 5) && (!strcmp(idtable.table[i].prefix, prefix)))
			{
				/*if (idtable.table[i].idtype == IT::F)
				...
				//strncpy(idtable.table[idtable.current_size].prefix, idtable.table[i].prefix, 5);
				return idtable.table[i].idxfirstLE;
